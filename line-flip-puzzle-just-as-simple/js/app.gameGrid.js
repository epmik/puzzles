...
app.gameGrid.prototype.nextRowState = function(row, stateToSwitch){
    
    for(var i = 0; i < this.elements.length; i++){
        var e = this.elements[i];
        
        if(e.rowIndex === row && e.state === stateToSwitch){
            e.nextState();        
        }
    }  
}

app.gameGrid.prototype.nextColumnState = function(column, stateToSwitch){
    
    for(var i = 0; i < this.elements.length; i++){
        var e = this.elements[i];
        
        if(e.columnIndex === column && e.state === stateToSwitch){
            e.nextState();        
        }
    }  
}
